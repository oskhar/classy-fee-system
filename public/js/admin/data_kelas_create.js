...
var Main = /*#__PURE__*/function (_Core) {
  ...
  function Main() {
    ...
    _this.inputNamaKelas = $("#nama_kelas");
  ...
  _createClass(Main, [{
    ...
    value: function setInputJurusan() {
      ...
      var url = "".concat(self.mainURL, "/api/jurusan/untuk-input-option");
          ...
          if (response.data.errors) {
            self.showInfoMessage(self.objectToString(response.data.errors), "pulihkan").then(function (result) {
              if (result.isConfirmed) {
                var _url = "".concat(self.mainURL, "/api/kelas/pulihkan");
                var _method = "put";
                var _dataBody = {
                  id_kelas: response.data.id_kelas
                };
                self.doAjax(_url, function (response) {
                  self.showSuccessMessage("Data ".concat(response.data.nama_kelas, " berhasil dipulihkan"));
                }, _dataBody, _method);
              }
            });
          } else {
            self.showSuccessMessage("Data ".concat(response.data.nama_kelas, " berhasil ditambahkan"));
          }
        }, dataBody, method);
      });
    }
